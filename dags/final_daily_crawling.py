    ...
    try:
        ...
        if len(blog_info) != 0:
            ...
            if len(store_info4["blog_list"]) != 0:
                ...
                upload_to_s3(
                    blogs_lists,
                    f"kakao/{keyword}/{now_date}/{store_id}_blogs.json",
                    "de-4-1-bucket",
                )
            else:
    ...
    for index in range(daily_index, len(keyword1), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                1, daily_keyword, timestamp_prev_day
        ...
        for index2 in range(daily_store_index, len(store_ids), 1):
            daily_get_visitor_reviews_blogs(
                daily_keyword, store_ids[index2], formatted_previous_day
            )  ##keyword, c_id, 20200202
            # mysql update
            update_store_info_daily(
                daily_keyword, timestamp_prev_day, store_ids[index2]
            )
            Variable.set("daily_store_index1", index2 + 1)
        Variable.set("daily_index1", index + 1)
...
def set_daily_keyword2(**kwargs):
    ...
    for index in range(daily_index, len(keyword2), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                2, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index2", index + 1)
...
def set_daily_keyword3(**kwargs):
    ...
    for index in range(daily_index, len(keyword3), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                3, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index3", index + 1)
...
def set_daily_keyword4(**kwargs):
    ...
    for index in range(daily_index, len(keyword4), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                4, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index4", index + 1)
...
def set_daily_keyword5(**kwargs):
    ...
    for index in range(daily_index, len(keyword5), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                5, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index5", index + 1)
...
def set_daily_keyword6(**kwargs):
    ...
    for index in range(daily_index, len(keyword6), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                6, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index6", index + 1)
...
def set_daily_keyword7(**kwargs):
    ...
    for index in range(daily_index, len(keyword7), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                7, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index7", index + 1)
...
def set_daily_keyword8(**kwargs):
    ...
    for index in range(daily_index, len(keyword8), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                8, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index8", index + 1)
...
def set_daily_keyword9(**kwargs):
    ...
    for index in range(daily_index, len(keyword9), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                9, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        ...
        Variable.set("daily_index9", index + 1)
...
def set_daily_keyword10(**kwargs):
    ...
    for index in range(daily_index, len(keyword10), 1):
        ...
        if daily_store_index == 0:
            execute_mysql_query_and_save_result(
                10, daily_keyword, timestamp_prev_day
            )  ## id 받아오기 => [datetime, [c_id_list]]
            logging.info("ENTER")
        c_li_list = json.loads(
            Variable.get("daily_store_ids10")
        )  # string => dict 작업 필요
        store_ids = c_li_list[1]
        now = c_li_list[0]
        for index2 in range(daily_store_index, len(store_ids), 1):
        ...
        Variable.set("daily_index10", index + 1)
...
default_args = {
    ...
    "start_date": datetime(2024, 2, 25, 3, 5),  # 한국시간 기준 12:10 $20240229부터 execute
