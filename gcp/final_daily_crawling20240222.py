...
['DDP', 'DMC', '강서한강공원','경리단길','고척돔', '광나루한강공원', '국립중앙박물관','난지한강공원','남산공원', '노들섬']+\
['노량진', '덕수궁길','동대문역', '뚝섬역','뚝섬한강공원','망원한강공원', '반포한강공원', '방배역', '북촌한옥마을']+\
['서촌', '성수카페거리', '성신여대입구역','수유리', '수유역','신논현역','쌍문동', '압구정로데오거리','여의도', '연남동']+\
[ '연신내역','왕십리역','용산가족공원','정동길','창동','청담동', '청량리', '청와대','해방촌']
# keywords2=['몽마르뜨공원','북서울꿈의숲','불광천', '서울숲공원','서리풀공원',
# '시청광장', '신도림역', '신림역', '신촌역','아차산', '양화한강공원', '어린이대공원', '외대앞', '여의도한강공원', 
# '월드컵공원','용리단길','이태원', '인사동', '익선동', '장지역', '장한평역', '천호역' ,
# '광화문광장','논현역','역삼역', '영등포', '용산역','이태원역','이화마을','청계산','총신대입구(이수)역', '충정로역',
    ...
    try:
        ...
        if len(blog_info)!=0:
            ...
            if len(store_info4['blog_list'])!=0:
                ...
                upload_to_s3(blogs_lists, f'kakao/airflow/{keyword}/{now_date}/{store_id}_blogs.json', 'de-4-1-bucket')  
            else:
...
dag = DAG(
    dag_id='daily_crawling_020240223',
    default_args=default_args,
    description='daily crawling dag',
    schedule_interval="10 3 * * 1-5", #주중에 실행
    catchup=False,
)

daily_get_keyword = PythonOperator(
    task_id='daily_get_keyword_task020240223',
